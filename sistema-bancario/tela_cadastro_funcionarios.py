...
def janelaCadastrarFuncionario():
    ...
    frame_baixo = tkinter.Frame(janela, width=300, height=260, relief='flat', bg='#feffff')
    ...
    cargo = tkinter.Label(frame_baixo, text="Senha *:", anchor=NW, font=('Ivy', 10), bg='#feffff', fg=c_pri)
    ...
    agencia.place(x=10, y=170)

    inputAgencia = tkinter.Entry(frame_baixo, width=25, font=("", 8), highlightthickness=1, relief='solid')
    inputAgencia.place(x=120, y=170)

    btnCadastrar = tkinter.Button(frame_baixo, text="Cadastrar", width=34, height=2, bg=c_sec, fg=branco, font=('Ivy 10 bold'), relief=FLAT, command=lambda: cadastro(inputNome.get(), inputmatricula.get(), inputCargo.get(), inputSalario.get(), inputAgencia.get(), inputSenha.get()))
    btnCadastrar.place(x=10, y=205)

    def cadastro(nome, matricula, cargo, salario, agencia, senha):
        
        funcionario = Funcionario(nome, matricula, cargo, salario, agencia, senha)
       
        sql = "INSERT INTO funcionario (nome, matricula, cargo, salario, agencia, senha) values(?,?,?,?,?,?)"
        cur.execute(sql, (funcionario.getNome, funcionario.getmatricula, funcionario.getCargo, 
                          funcionario.getSalario, funcionario.getAgencia, funcionario.getSenha))
